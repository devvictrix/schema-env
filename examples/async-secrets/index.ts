...
async function main() {
  ...
  try {
    ...
    console.log(`Database URL Loaded: ${env.DATABASE_URL ? "Yes" : "No"}`);
    console.log(`API Key Loaded: ${env.THIRD_PARTY_API_KEY ? "Yes" : "No"}`);
    console.log(`Feature Flag X: ${env.FEATURE_FLAG_X}`); // Should be false from process.env
